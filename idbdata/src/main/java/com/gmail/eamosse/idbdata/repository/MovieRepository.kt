...
class MovieRepository : KoinComponent {
    ...
    suspend fun getMoviesFromCategory(categoryId: Int): Result<List<Movie>> {
        return when(val result = online.getMoviesFromCategory(categoryId)) {
            is Result.Succes -> {
                // On utilise la fonction map pour convertir les catégories de la réponse serveur
                // en liste de categories d'objets de l'application
    ...
    suspend fun getMovieWithId(movieId: Int): Result<MovieDetail> {
        return when(val result = online.getMovieWithId(movieId)) {
            is Result.Succes -> {
                val movie = result.data.toMovieDetail()
                Result.Succes(movie)
            }
            is Result.Error -> result
        }
    ...
    suspend fun getTrendingActors(): Result<List<TrendingActor>> {
        return when(val result = online.getTrendingActors()) {
            is Result.Succes -> {
                // On utilise la fonction map pour convertir les catégories de la réponse serveur
                // en liste de categories d'objets de l'application
